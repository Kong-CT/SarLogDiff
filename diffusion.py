...
class GaussianDiffusion(nn.Module):
    ...
    def _extract(coef:torch.Tensor, t:torch.Tensor, x_shape:tuple) -> torch.Tensor:
        ...
        t = t.to(coef.device)  # Ensure t is on the same device as coef
        chosen = coef[t]       # Perform indexing

        neo_shape = [x_shape[0]] + [1] * (len(x_shape) - 1)
        return chosen.reshape(neo_shape)
    ...
    def q_sample(self, x_0: torch.Tensor, t: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        sample from q(x_t|x_0)
        """
        # device = x_0.device  # Get the device from x_0
        # eps = torch.randn_like(x_0, requires_grad=False, device=device)  # Ensure eps is on the same device as x_0
        # # Ensure all coefficients are on the same device as x_0
        # sqrt_alphas = self._extract(self.sqrt_alphas_bar.to(device), t, x_0.shape) * x_0
        # sqrt_one_minus_alphas = self._extract(self.sqrt_one_minus_alphas_bar.to(device), t, x_0.shape) * eps
        # return sqrt_alphas + sqrt_one_minus_alphas, eps
        device = x_0.device
        eps = torch.randn_like(x_0, requires_grad=False, device=device)
        sqrt_alphas = self._extract(self.sqrt_alphas_bar.to(device), t, x_0.shape) * x_0
        sqrt_one_minus_alphas = self._extract(self.sqrt_one_minus_alphas_bar.to(device), t, x_0.shape) * eps
        return sqrt_alphas + sqrt_one_minus_alphas, eps
    ...
    def p_mean_variance(self, x_t:torch.Tensor, t:torch.Tensor, **model_kwargs) -> tuple[torch.Tensor, torch.Tensor]:
        ...
        if model_kwargs == None:
            model_kwargs = {}
        B, C = x_t.shape[:2]
        assert C == 1, "x_t must have 1 channel" 
        assert t.shape == (B,)
    ...
    def sample(self, shape:tuple, **model_kwargs) -> torch.Tensor:
        ...
        if model_kwargs == None:
            model_kwargs = {}
        # Ensure the channel dimension is 1
        single_channel_shape = (shape[0], 1, shape[2], shape[3])
        x_t = torch.randn(single_channel_shape, device = self.device)
